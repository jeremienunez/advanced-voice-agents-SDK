...
export const envFieldDefinitions: EnvFieldDefinition[] = [
  ...
  field("BUILDER_INFRA_COMPUTE_TARGET", "infra", "Compute target", "IaC compute target emitted by the planner.", ["local", "k3s", "kubernetes", "vm"], "local"),
...
function requirementStates(fields: EnvFieldState[]): EnvRequirementState[] {
  return [
    ...
    requirement(
      ...
      "infra",
      "Learning runtime",
      "REDIS_URL and Temporal worker settings are required for automatic post-session learning.",
      ["REDIS_URL", "TEMPORAL_ADDRESS", "TEMPORAL_TASK_QUEUE"],
      fields,
      "recommended",
      "all",
    ),
